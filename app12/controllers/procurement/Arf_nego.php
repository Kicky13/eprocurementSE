...
class Arf_nego extends CI_Controller {
    ...
    {
        $data['list'] = $this->m_arf_response->view('arf_response')->where('t_arf_response.id not in (select arf_response_id from t_arf_nego where status = 0 group by arf_response_id)')->get();
