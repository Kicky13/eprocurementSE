    ...
    <div class="content-wrapper">
        <div class="content-header row">
            <div class="content-header-left col-md-6 col-12 mb-1">
                <h3 id="arfn_title1" class="content-header-title onprepare">
                    <?= lang("Persiapan Notifikasi Amendment", "Amendment Notification Preparation") ?>
                </h3>
                <h3 id="arfn_title2" class="content-header-title onprogress" style="display: none;">
                    <?=lang("Progres Notifikasi Amendment", "Amendment Notification On Progress")?>
                </h3>
            </div>
            <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-12">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"><?= lang("Persiapan Notifikasi Amendment", "Amendment Notification Preparation") ?></li>
                    </ol>
                </div>
            </div>
        </div>
        <div class="content-detached">
            <div class="content-body" id="container_list">
                <input type="hidden" name="arf_no" id="arf_no">
                <div class="card">
                    <div class="card-content">
                        ...
                        <div class="card-body">
                            <div class="col-12 form-row onprepare">
                                <table id="list1" class="table table-striped table-bordered table-hover table-no-wrap display" width="100%">
                                    <tfoot>
                                        <tr>
                                            <th>No</th>
                                            <th></th>
                            ...
                            <div class="col-12 form-row onprogress">
                                <table id="list2" class="table table-striped table-bordered table-hover table-no-wrap  display" width="100%">
                                    <tfoot>
                                        <tr>
                                            <th>No</th>
                                            <th></th>
            ...
            <div class="modal-dialog modal-lg modal-form">
                ...
                <form id="modal_sop_form" class="modal-content" action="javascript:;" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary btn-modif"><?= lang("Proses", "Add") ?></button>
                    </div>
                </form>
            </div>
        </div>
...
<div id="modal_upload" class="modal fade" data-backdrop="static" role="dialog">
    <div class="modal-dialog">
        <form id="modal_upload_form" class="modal-content" action="javascript:;" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="form-group">
                    <label class="control-label" for="field-1"><?= lang("Pilih File", "File") ?></label>
                    <input type="file" name="modal_upload_file" id="modal_upload_file" required/>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-secondary" data-dismiss="modal"><?= lang("Batal", "Cancel") ?></button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#list1 tfoot th').each(function (i) {
          var title = $('#list1 thead th').eq(i).text();
          if ($(this).text() == 'No') {
            $(this).html('');
        ...
        $('#list2 tfoot th').each(function (i) {
          var title = $('#list2 thead th').eq(i).text();
          if ($(this).text() == 'No') {
            $(this).html('');
          } else if ($(this).text() == 'Action') {
            $(this).html('');
        ...
        $(list2.table().container()).on('keyup', 'tfoot input', function () {
          list2.column($(this).data('index')).search(this.value).draw();
        });
        ...
        $('#modal_sop_form').submit(function(e) {
            ...
            obj['modal_sop_uom_desc_1'] = $('#modal_sop_uom_1 option:selected').text().split(' - ')[1];
            if ($('#modal_sop_uom_2').val() != '') {
                obj['modal_sop_uom_desc_2'] = $('#modal_sop_uom_2 option:selected').text().split(' - ')[1];
            }
            ...
            if (resp.length > 0) {
                swal('<?= __('warning') ?>', resp[1], 'warning');
            } else {
                ...
                set_item_proc();
                //swal('<?= __('success') ?>', resp[1], 'success');
            }
            //stop(elm);
        });
        ...
        <?php
            if (isset($data_rev) && $data_rev && strcmp($data_rev->flag, 'after_review') == 0) { ?>
                ...
                $('#notif_comdate_input').datetimepicker({format:'YYYY-MM-DD HH:mm'});
    ...
    function show_prepare() {
        $('.onprepare').show();
        $('.onprogress').hide();
        $('#list1').DataTable().draw();
    }

    function show_progress() {
        $('.onprepare').hide();
        $('.onprogress').show();
        $('#list2').DataTable().draw();
    }
    ...
    function delete_ul(id) {
        swalConfirm('Amendment Notification Preparation', '<?= __('confirm_delete') ?>', function() {
            var obj = {};
            obj.id = id;
            obj.po = $('#param_po').val();
    ...
    function preview(id) {
        ...
        // } else {
            window.open('<?= base_url()?>procurement/arf_notif_preparation/download_file/' + $('#param_po').val() + '_' + $('#param_notif').val() + '_' + id, '_blank');
        // }
    }
    ...
    function sop_del(index) {
        ...
        swalConfirm('ARF Notification Preparation', '<?= __('confirm_delete') ?>', function() {
            ...
            if (sop_arf[0]['modal_sop_qty_type'] == 2) {
                for (var i = 0; i < sop_arf.length; i++) {
                    if (sop_arf[i]['status'] == 2 || sop_arf[i]['status'] == 1 || (sop_arf[i]['status'] == 0 && sop_arf[i]['modal_sop_id'] != 0)) {
                        ...
                        sop_arf_list[sal_counter][4] = sop_arf[i]['modal_sop_uom_1'] + ' - ' + sop_arf[i]['modal_sop_uom_desc_1'];
                        sop_arf_list[sal_counter][5] = (sop_arf[i]['modal_sop_uom_2'] == null ? '-' : sop_arf[i]['modal_sop_qty_2']);
                        sop_arf_list[sal_counter][6] = (sop_arf[i]['modal_sop_uom_2'] == null ? '-' : sop_arf[i]['modal_sop_uom_2'] + ' - ' + sop_arf[i]['modal_sop_uom_desc_2']);
                        sop_arf_list[sal_counter][7] = Localization.boolean(sop_arf[i]['modal_sop_item_mod'], 'Yes' , 'No');
                        ...
                        sop_arf_list[sal_counter][10] = sop_arf[i]['modal_sop_sub_acc'] + ' - ' + sop_arf[i]['modal_sop_sub_acc_name'];
                        if (obj.type != 3)
                            sop_arf_list[sal_counter][11] = "<button class='btn btn-sm btn-warning' onclick='sop_mdl_edit(" + i + ")'>Edit</button>&nbsp;<button class='btn btn-sm btn-danger' onclick='sop_del(" + i + ")'>Delete</button>";
                        else
                            sop_arf_list[sal_counter][11] = "<button class='btn btn-sm btn-warning' onclick='sop_mdl_edit(" + i + ")'>Detail</button>";
                        sal_counter++;
                    }
                }
            } else {
    ...
    function send_data(type) {
        if ($('#notif_comdate_input').val() == '' && type != 1) {
            ...
            if (type == 2) {
                swalConfirm('ARF Notification Preparation', '<?= __('confirm_submit') ?>', function() {
                    send_data_ajax(type, obj);
                });
            } else if (type == 1) {
                send_data_ajax(type, obj);
            }
